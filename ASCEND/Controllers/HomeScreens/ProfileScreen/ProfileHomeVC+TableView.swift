...
extension ProfileHomeVC: UITableViewDataSource{
...
extension ProfileHomeVC: UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0{
            let header: ProfileHomeHeaderView  = Bundle.main.loadNibNamed(ProfileHomeHeaderView.className, owner: self, options: nil)?.first as! ProfileHomeHeaderView
            ...
            myMutableString = NSMutableAttributedString()
            myMutableString.append(self.customMethodManager?.provideUnderlineAttributedText(text: "\(ConstantTexts.EditLT)", font: AppFont.light.size(AppFontName.poppins, size: 12), color: AppColor.whiteColor) ?? NSMutableAttributedString())
            
            // *** Apply attribute to string ***
            myMutableString.addAttribute(NSAttributedString.Key.paragraphStyle, value:paragraphStyleCenter, range:NSMakeRange(0, myMutableString.length))
            // *** Set Attributed String to your label ***
            
            header.btnEditRef.setAttributedTitle(myMutableString, for: .normal)
            ...
            header.btnEditProfileRef.addTarget(self, action: #selector(btnEditProfileTapped), for: .touchUpInside)
