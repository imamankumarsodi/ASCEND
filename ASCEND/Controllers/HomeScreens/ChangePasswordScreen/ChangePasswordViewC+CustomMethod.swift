//
//  ChangePasswordViewC+CustomMethod.swift
//  ASCEND
//
...
extension ChangePasswordViewC{
    ...
    internal func navSetup(){
        ...
        super.setupNavigationBarTitle(AppColor.appColor, ConstantTexts.ChangePasswordLT, leftBarButtonsType: [.back], rightBarButtonsType: [.empty])
    }
...
extension ChangePasswordViewC{
    ...
    internal func validateFields(validHandler: @escaping ( String, Bool,UIView?) -> Void){
        ...
        if !validationMethodManager!.checkEmptyField(self.txtCurrentPassword.text!.trimmingCharacters(in: .whitespacesAndNewlines)) {
            
            validHandler( ConstantTexts.EnterCurrntPasswordALERT, false, self.txtCurrentPassword)
            return
        }
