...
extension SBaseViewController:UIImagePickerControllerDelegate,UINavigationControllerDelegate  {
    ...
    internal func openCamera() {
        if(UIImagePickerController .isSourceTypeAvailable(UIImagePickerController.SourceType.camera)) {
            ...
            let alert = UIAlertController(title: ConstantTexts.AppName, message:ConstantTexts.CameraALERT, preferredStyle: UIAlertController.Style.alert)
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let choosenImage = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
            ...
            let item = DocumentDataModel(data: Data(), type: ".png", withName: "OtherDocs", fileName: String(), mimeType: "image/png")
