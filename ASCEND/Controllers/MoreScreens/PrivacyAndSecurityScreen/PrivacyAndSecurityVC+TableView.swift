//
//  PrivacyAndSecurityVC+TableView.swift
//  ASCEND
//
...
extension PrivacyAndSecurityVC: UITableViewDataSource{
...
extension PrivacyAndSecurityVC: UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0{
         ...
         self.navigationController?.pushViewController(vcS, animated: true)//aage bhej do
            ...
            guard let vcS = AppStoryboard.homeSB.instantiateViewController(withIdentifier: VisibleViewC.className) as? VisibleViewC  else{
                print("No VisibleViewC found..")
                return
            }
            self.navigationController?.pushViewController(vcS, animated: true)//aage bhej do
